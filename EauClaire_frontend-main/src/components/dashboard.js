  ...
  useEffect(() => {
    ...
    Promise.all(
      selectedRegions.map((region) =>
        fetch(`https://eauclaire.online/api/qualite-reg?region=${region}`).then(
      ...
      .then((datas) => {
        ...
        const datasets = datas.map((data, index) => {
          ...
          return {
            ...
            data: qualities,
            borderColor: `rgba(${(index * 60) % 255}, ${
              (index * 120) % 255
            }, 200, 1)`,
            backgroundColor: `rgba(${(index * 60) % 255}, ${
              (index * 120) % 255
            }, 200, 0.2)`,
            fill: true,
          };
    ...
    Promise.all(
      selectedRegions.map((region) =>
        fetch(
          `https://eauclaire.online/api/biodiversite?region=${region}&year=${selectedYear}`
        ).then((response) => response.json())
      )
    )
      .then((datas) => {
    ...
    Promise.all(
      selectedRegions.map((region) =>
        fetch(
          `https://eauclaire.online/api/niveau?region=${region}&year=${selectedYear}`
        ).then((response) => response.json())
      )
    )
      .then((datas) => {
      ...
      .catch((error) =>
        console.error("Error fetching water risk data:", error)
      );
    ...
    <div style={{ margin: "50px" }}>
      <div style={{ marginBottom: "20px" }}>
        ...
        <div className="dropdown-container">
          <button className="dropdown-button">Select Regions</button>
          ...
          <Bar
            ...
            options={{
              ...
              plugins: {
                ...
                title: {
                  display: true,
                  text: `Species Count and Extinction Risk (${selectedYear})`,
                },
              },
            }}
          />
          ...
          <Line
            ...
            options={{
              ...
              plugins: {
                ...
                title: {
                  display: true,
                  text: "Water Quality Index (2000-2050)",
                },
              },
            }}
          />
