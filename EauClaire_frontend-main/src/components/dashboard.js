...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetch(`http://127.0.0.1:8000/api/niveau?year=2000`)
  ...
  useEffect(() => {
    ...
    Promise.all(selectedRegions.map(region =>
      fetch(`http://127.0.0.1:8000/api/qualite-reg?region=${region}`)
        .then(response => response.json())
    )).then(datas => {
    ...
    Promise.all(selectedRegions.map(region =>
      fetch(`http://127.0.0.1:8000/api/biodiversite?region=${region}&year=${selectedYear}`)
        .then(response => response.json())
    )).then(datas => {
      const rivers = [];
      const speciesCount = [];
    ...
    Promise.all(selectedRegions.map(region =>
      fetch(`http://127.0.0.1:8000/api/niveau?region=${region}&year=${selectedYear}`)
        .then(response => response.json())
    )).then(datas => {
      const regions = [];
      const droughtRisks = [];
